...
Ray Camera::generateRay(const glm::ivec2 &pixel_coord, const glm::vec2 &offset) {
    glm::vec2 ndc = (glm::vec2(pixel_coord) + offset) / glm::vec2(film.getWidth(), film.getHeight());
    ndc.y = 1.f - ndc.y;
    // [0, 1] --> [-1, 1]
    ndc = ndc * 2.f - 1.f;
    glm::vec4 clip { ndc, 0, 1 };
    glm::vec4 world = world_from_camera * camera_from_clip * clip;
    return Ray {
        pos,
        glm::normalize(glm::vec3(world))
    };
}
